...
function blueNoise(width, height, scale, intensity, colorNoise) {
  ...
  const textureSize = 186;
  const channels = colorNoise ? 3 : 1;
  const noise = new Uint8ClampedArray(width * height * channels);

  // Вспомогательная функция для вычисления расстояния с учётом тайлинга
  ...
  // Генерация синего шума с помощью void-and-cluster
  function generateBlueNoiseTexture(size) {
    const points = [];
    const numPoints = Math.floor(size * size * 0.1); // 10% заполнения

    // Шаг 1: Создаём начальные случайные точки
    for (let i = 0; i < numPoints; i++) {
      points.push({
        x: Math.random() * size,
        y: Math.random() * size
    ...
    // Шаг 2: Void-and-cluster итерации
    for (let iter = 0; iter < 5; iter++) {
      // Находим "пустоты" (voids)
      const gridSize = 8;
      const cellSize = size / gridSize;
      let maxVoid = { score: -Infinity, x: 0, y: 0 };
      ...
      for (let gy = 0; gy < gridSize; gy++) {
        for (let gx = 0; gx < gridSize; gx++) {
          ...
          for (const p of points) {
            const dist = toroidalDistance(cx, cy, p.x, p.y, size);
            minDist = Math.min(minDist, dist);
          }
          
          if (minDist > maxVoid.score) {
            maxVoid = { score: minDist, x: cx, y: cy };
          }
        }
      }

      // Находим "кластеры" (clusters)
      let minCluster = { score: Infinity, index: -1 };
      for (let i = 0; i < points.length; i++) {
        let minDist = Infinity;
        for (let j = 0; j < points.length; j++) {
          if (i !== j) {
            const dist = toroidalDistance(points[i].x, points[i].y, points[j].x, points[j].y, size);
            minDist = Math.min(minDist, dist);
          }
        }
        if (minDist < minCluster.score) {
          minCluster = { score: minDist, index: i };
        }
      }

      // Заменяем точку из кластера на точку в пустоте
      if (minCluster.index !== -1) {
        points[minCluster.index] = { x: maxVoid.x, y: maxVoid.y };
      }
    }

    // Шаг 3: Преобразуем точки в текстуру
    const texture = new Uint8Array(size * size);
    for (let y = 0; y < size; y++) {
      for (let x = 0; x < size; x++) {
        let minDist = Infinity;
        for (const p of points) {
          const dist = toroidalDistance(x, y, p.x, p.y, size);
          minDist = Math.min(minDist, dist);
        }
        // Нормализуем расстояние в [0, 255]
        const value = Math.min(255, Math.floor((minDist / (size * 0.1)) * 255));
        texture[y * size + x] = value;
      }
    }
    return texture;
  }

  // Генерируем текстуры для каждого канала (для colorNoise)
  const textures = [];
  if (colorNoise) {
    for (let i = 0; i < 3; i++) {
      textures.push(generateBlueNoiseTexture(textureSize));
    }
  } else {
    textures.push(generateBlueNoiseTexture(textureSize));
  }

  // Заполняем выходной массив с учётом масштабирования
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      const nx = (x * scale) % textureSize;
      const ny = (y * scale) % textureSize;
      const idx = Math.floor(ny) * textureSize + Math.floor(nx);

      if (colorNoise) {
        for (let c = 0; c < 3; c++) {
          let value = textures[c][idx];
          value = Math.min(255, Math.max(0, value * intensity));
          // Добавляем небольшие вариации
          value = Math.min(255, Math.max(0, value + (Math.random() * 20 - 10)));
          noise[(y * width + x) * 3 + c] = value;
        }
      } else {
        let value = textures[0][idx];
        value = Math.min(255, Math.max(0, value * intensity));
...
self.onmessage = function (e) {
  try {
    ...
    const dataArray = new Uint8ClampedArray(processedData.data);
    self.postMessage(
      { data: dataArray, width: processedData.width, height: processedData.height },
      [dataArray.buffer]
    );  } catch (error) {
